...
export class HelperClient {
  ...
  private async createSession() {
    // Preserve the anonymousSessionId by passing the current token when recreating the session
    const params = this.token ? { ...this.sessionParams, currentToken: this.token } : this.sessionParams;
    const { token, supabaseUrl, supabaseAnonKey } = await this.sessions.create(params);
    this.token = token;
