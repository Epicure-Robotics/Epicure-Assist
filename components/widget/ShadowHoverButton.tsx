...
export default function ShadowHoverButton({
  ...
  return (
    <div className="relative ml-2">
      ...
      <div
        className={`absolute left-0 top-0 h-8 w-8 rounded-md ${isEpicureSiteTheme ? "bg-black" : "bg-bright-foreground"} transition-all duration-300 ease-in-out ${isLoading ? "opacity-50" : ""}`}
      />
    </div>
  );
}
