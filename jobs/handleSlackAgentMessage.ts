...
export const handleSlackAgentMessage = async ({
  ...
  const knowledgeEntryUser =
    confirmKnowledgeBaseEntry && slackUserId
      ? await findUserViaSlack(assertDefined(mailbox.slackBotToken), slackUserId)
      : null;

  if (confirmKnowledgeBaseEntry && slackUserId && knowledgeEntryUser?.email === "bharat@openvision.engineering") {
    const { message_ts: ts } = await client.chat.postEphemeral({
      channel: agentThread.slackChannel,
      thread_ts: agentThread.threadTs,
      user: slackUserId,
      text: `Knowledge base entry to save:\n${confirmKnowledgeBaseEntry.args.entry}`,
      blocks: [
        {
          type: "section",
