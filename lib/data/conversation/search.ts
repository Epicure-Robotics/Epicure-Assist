...
export const searchConversations = async (
  ...
  const isOpenOrWaitingTicketsOnly =
    filters.status?.length === 1 && statusFilter
      ? ["open", "waiting_on_customer", "check_back_later"].includes(statusFilter)
      : false;
  const orderBy =
    sort === "oldest"
      ? [asc(orderByField)]
        ...
        : sort === "created_desc"
          ...
          : sort === "updated_asc"
            ...
            : sort === "updated_desc"
              ...
              : [desc(orderByField)];
  if (sort === "highest_value" && isOpenOrWaitingTicketsOnly && !filters.search) {
