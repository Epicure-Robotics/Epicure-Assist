...
export const updateOriginalConversation: typeof updateConversation = async (id, options, tx = db) => {
  return await updateConversation(id, options, tx);
};
...
export const updateConversation = async (
  ...
  // If assigning to a human, disable AI auto-response
  // Removed per user request to allow monitoring AI responses
  /*
  if (dbUpdates.assignedToId) {
  ...
  if (current.status !== "closed" && updatedConversation?.status === "closed") {
    ...
    if (updatedConversation.conversationProvider === "gmail") {
      const hasPendingEmail = await tx.query.conversationMessages.findFirst({
        columns: { id: true },
        where: and(
          eq(conversationMessages.conversationId, updatedConversation.id),
    ...
    // Schedule closure notification email if feature is enabled and closed by staff
    if (byUserId) {
      const closureMailbox = await getMailbox();
      if (closureMailbox?.closedThreadEmailEnabled) {
        await triggerEvent(
          "conversations/closed-thread-email",
          { conversationId: updatedConversation.id, closedByUserId: byUserId },
          { sleepSeconds: 24 * 60 * 60 }, // 24 hours
        );
      }
    }
  }
  ...
  if (updatedConversation && !skipRealtimeEvents) {
    ...
    if (byUserId && updatesToLog.length > 0) {
      ...
      if (current.assignedToId !== updatedConversation.assignedToId) {
        ...
        // Send web notification to newly assigned team member
        if (updatedConversation.assignedToId) {
          notificationEvents.push(
            triggerEvent("notifications/create-web-notification", {
              conversationId: updatedConversation.id,
              type: "assignment_change" as const,
              triggeredByUserId: byUserId || undefined,
            }),
          );

          // Send assignment email if enabled
          notificationEvents.push(
            triggerEvent("conversations/send-assignment-email", {
              conversationId: updatedConversation.id,
              newAssigneeId: updatedConversation.assignedToId,
              triggeredByUserId: byUserId,
            }),
          );
        }
