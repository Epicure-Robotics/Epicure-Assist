...
/**
 * Gets the most recent bot message in a DM channel to use as a thread parent.
 * This allows all notification messages to be grouped in the same thread.
 */
export const getLatestBotMessageInDM = async (
  token: string,
  channelId: string,
  botUserId: string,
): Promise<string | null> => {
  try {
    ...
    const historyResponse = await client.conversations.history({
      channel: channelId,
      limit: 20, // Check last 20 messages to find a bot message
    });

    if (!historyResponse.ok || !historyResponse.messages) {
      throw new Error(`Failed to fetch conversation history: ${historyResponse.error}`);
    }

    // Find the most recent message sent by the bot (not a reply in a thread)
    const latestBotMessage = historyResponse.messages.find(
      (msg) => msg.user === botUserId && !msg.thread_ts, // Top-level messages only
    );

    return latestBotMessage?.ts ?? null;
  } catch (error) {
    captureExceptionAndLog(error);
    return null;
  }
...
export const sendSlackDM = async (
  ...
  try {
    ...
    // Send the message to the DM channel, optionally as a thread reply
    const messageResponse = await client.chat.postMessage({
      channel: dmChannelId,
      text,
      blocks,
      thread_ts: threadTs, // If provided, sends as a threaded reply
    });

    if (!messageResponse.ok || !messageResponse.message?.ts) {
      throw new Error(`Failed to send DM: ${messageResponse.error}`);
    }

    return {
      channelId: dmChannelId,
      messageTs: messageResponse.message.ts,
    };
  } catch (error) {
    captureExceptionAndLog(error);
    return null;
