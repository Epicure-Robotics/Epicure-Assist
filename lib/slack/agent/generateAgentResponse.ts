...
export const generateAgentResponse = async (
  ...
  console.log(`[generateAgentResponse] Starting agent response for mailbox: ${mailbox.name}`);
  console.log(`[generateAgentResponse] User message:`, messages[messages.length - 1]?.content);
  ...
  const tools: Record<string, Tool> = {
    ...
    searchTickets: tool({
      description:
        "Search tickets/conversations with various filtering options. Use `nextCursor` to paginate through results; if it's set then pass it as `cursor` to get the next page of results. IMPORTANT: When doing a keyword search, ONLY set the 'search' parameter and leave other filters unset unless specifically requested by the user.",
      parameters: searchToolSchema,
      execute: async (input) => {
        showStatus(`Searching tickets...`, { toolName: "searchTickets", parameters: input });

        // Clean up the input: remove default/empty values that would overly restrict the search
        const cleanedInput: typeof input = Object.fromEntries(
          Object.entries(input).filter(([key, value]) => {
            // Keep these if they have meaningful values
            if (key === "search" && value) return true;
            if (key === "limit" && value) return true;
            if (key === "cursor" && value) return true;
            if (key === "sort" && value) return true;
            if (key === "status" && value && Array.isArray(value) && value.length > 0) return true;
            if (key === "assignee" && value && Array.isArray(value) && value.length > 0) return true;
            if (key === "customer" && value && Array.isArray(value) && value.length > 0) return true;

            // Remove filters with "falsy" or default values
        ...
        console.log(`[searchTickets] Original params:`, JSON.stringify(input, null, 2));
        console.log(`[searchTickets] Cleaned params:`, JSON.stringify(cleanedInput, null, 2));

        try {
          const { list } = await searchConversations(mailbox, cleanedInput);
          ...
          console.log(`[searchTickets] Found ${response.tickets.length} tickets`);
          console.log(
            `[searchTickets] Results:`,
            response.tickets.map((t) => `#${t.id} (${t.slug}): ${t.subject}`).join("\n  "),
          );
          return response;
        } catch (error) {
    ...
    generateShareableLink: tool({
      description:
        "Generate a public shareable link for a ticket that can be viewed by anyone without logging in. The link expires after 30 days.",
      parameters: z.object({
        id: z
          .union([z.string(), z.number()])
          .describe(
      ...
      execute: async ({ id }) => {
        showStatus(`Generating shareable link...`, { toolName: "generateShareableLink", parameters: { id } });
        console.log(`[generateShareableLink] Looking up ticket with ID: ${id}`);
        const conversation = await findConversation(id);
        if (!conversation) {
          console.log(`[generateShareableLink] Ticket not found: ${id}`);
        ...
        console.log(`[generateShareableLink] Generated link for ticket #${conversation.id} (${conversation.slug})`);
        console.log(`[generateShareableLink] Public link: ${publicLink}`);

        return {
          message: `Public link generated (expires in 30 days)`,
          link: publicLink,
        };
      },
    }),
...
- When asked to create a shareable link for a ticket:
  * If given a ticket ID or URL, use generateShareableLink directly with that ID
  * If given a subject line, keyword, or description, first use searchTickets to find the ticket, then use generateShareableLink with the ticket ID from the search results
  * The link allows anyone to view the conversation without logging in and expires after 30 days
...
Do not use em dashes (—) in your response.`,
  ...
  // Log the final response and tool calls
  console.log(`[generateAgentResponse] Agent completed with ${result.steps?.length || 0} steps`);
  console.log(
    `[generateAgentResponse] Tool calls made:`,
    result.steps?.flatMap((step) => step.toolCalls?.map((c) => c.toolName) ?? []).join(", ") || "none",
  );
  console.log(`[generateAgentResponse] Final response text:`, result.text.substring(0, 200));
