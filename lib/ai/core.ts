...
export const generateEmbedding = async (
  ...
  const embeddingModels = Array.from(new Set([EMBEDDING_MODEL, EMBEDDING_MODEL_FALLBACK]));
...
export const generateCompletion = ({
  ...
  retryOnPromptLengthError(shortenPromptBy, { system, prompt, messages }, (prompt) =>
    generateText({
      model: openai(model, { reasoningEffort: model === O4_MINI_MODEL ? "low" : undefined, structuredOutputs: false }),
      temperature,
      ...options,
      ...prompt,
      experimental_telemetry: {
...
export const cleanUpTextForAI = (text: string | null) => {
  ...
  const withoutEmailArtifacts = withoutBase64
    .replace(/https?:\/\/urldefense(?:\.proofpoint)?\.com\/[^\s)\]]+/gi, " ")
    .replace(/https?:\/\/[^\s)\]]*inline-attachments\/[^\s)\]]+/gi, " ")
    .replace(/\[?inline-attachments\/[^\s)\]]+\]?/gi, " ")
    .replace(/\bAI-generated content may be incorrect\./gi, " ");
  const withSingleLineBreaks = withoutEmailArtifacts.replace(/\n{2,}/g, "\n");
