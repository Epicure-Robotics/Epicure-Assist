...
export const convertConversationMessageToRaw = async (
  ...
  // Check if this is an HTML template (stored in htmlBody)
  if (email.htmlBody) {
    html = email.htmlBody;
    text = htmlToText(email.htmlBody);
  } else if (email.role === "ai_assistant") {
    ...
    const user = email.userId ? await getFullProfileById(email.userId) : null;

    // Only add signature if user has displayName and hasn't disabled the signature
    if (html && user?.displayName && !user?.preferences?.disableEmailSignature) {
      html += `<p>Best,<br />${getFirstName(user)}<br /><br /><a href="https://docs.google.com/forms/d/e/1FAIpQLSd0l60SUS24VPsHmeVsLsBPtcwCXaLY-jjYwSgtZV3dgENB6w/viewform?usp=header" target="_blank" rel="noopener noreferrer">Rate our support: 😀 · 😐 · 🙁</a></p>`;
    }
  ...
  const message = new MailComposer({
    from: emailFrom,
    to: email.emailTo ?? email.conversation.emailFrom,
