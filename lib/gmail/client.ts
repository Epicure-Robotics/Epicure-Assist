...
export const subscribeToMailbox = async (client: GmailClient) => {
  return await client.users.watch({
    userId: "me",
    requestBody: {
      // Watch all messages (including archived) to capture complete conversation threads
      // Filtering is done in handleGmailWebhookEvent instead (SENT, categories, etc.)
