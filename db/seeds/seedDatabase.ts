...
const seedHelpArticles = async () => {
  ...
  for (const article of helpArticlesData) {
    await db.insert(websitePages).values({
      ...
      rawHtml: `<html><head><title>${article.title}</title></head><body><h1>${article.title}</h1><p>See markdown snapshot in Epicure Assist knowledge seed.</p></body></html>`,
      markdown: article.markdown,
      metadata: {
        title: article.title,
        description: `Epicure public site snapshot: ${article.title}`,
      },
      createdAt: subDays(new Date(), 1),
      updatedAt: new Date(),
    });
