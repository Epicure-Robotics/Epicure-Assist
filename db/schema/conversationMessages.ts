...
type MessageStatus = "queueing" | "sent" | "failed" | "draft" | "staff_draft" | "discarded";
...
export const DRAFT_STATUSES: Partial<MessageStatus>[] = ["draft", "staff_draft", "discarded"];
...
export const conversationMessages = pgTable(
  "messages",
  {
    ...
    // For staff drafts (shared across team)
    draftAuthorId: text("draft_author_id"),
    draftEditedAt: timestamp("draft_edited_at", { withTimezone: true }),
    draftVersion: bigint("draft_version", { mode: "number" })
      .notNull()
      .default(0),
  },
  (table) => [
    ...
    index("idx_messages_replied_by_filter")
      ...
      .concurrently(),
    // Index for querying staff drafts by conversation
    index("idx_messages_staff_drafts")
      .on(table.conversationId, table.status, table.draftEditedAt)
      .where(assertDefined(and(isNull(table.deletedAt), eq(table.status, "staff_draft")))),
    unique("messages_gmail_message_id_unique").on(table.gmailMessageId),
  ],
).enableRLS();
