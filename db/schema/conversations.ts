...
export const conversations = pgTable(
  "conversations_conversation",
  {
    ...
    status: text().$type<"open" | "waiting_on_customer" | "closed" | "spam" | "check_back_later" | "ignored">(),
  ...
  (table) => [
    ...
    index("conversations_issue_subgroup_id_idx").on(table.issueSubgroupId),
...
export const conversationsRelations = relations(conversations, ({ one, many }) => ({
  ...
  issueSubgroup: one(issueSubgroups, {
    fields: [conversations.issueSubgroupId],
    references: [issueSubgroups.id],
  }),
