...
export const mailboxes = pgTable(
  "mailboxes_mailbox",
  {
    ...
    closedThreadEmailEnabled: boolean().notNull().default(false),
    weekendAutoReplyEnabled: boolean().notNull().default(false),
    weekendAutoReplyMessage: text(),
    holidayAutoReplyEnabled: boolean().notNull().default(false),
    holidayAutoReplyMessage: text(),
    preferences: jsonb()
      .$type<{
        autoRespondEmailToChat?: "draft" | "reply" | null;
        disableTicketResponseTimeAlerts?: boolean;
        archiveGmailOnReply?: boolean;
      }>()
      .default({}),
  },
  (table) => [
