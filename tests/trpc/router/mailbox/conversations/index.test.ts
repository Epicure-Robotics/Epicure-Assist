...
describe("conversationsRouter", () => {
  describe("list", () => {
    ...
    it("defaults to oldest-first ordering", async () => {
      ...
      const { conversation: olderConversation } = await conversationFactory.create({
        createdAt: olderDate,
        lastMessageAt: olderDate,
      });
      const { conversation: newerConversation } = await conversationFactory.create({
        createdAt: newerDate,
      ...
      expect(result.defaultSort).toBe("oldest");
      expect(result.conversations.map((c) => c.slug)).toEqual([olderConversation.slug, newerConversation.slug]);
    });
    ...
    it("sorts by platformCustomers.value with nulls last", async () => {
      ...
      const caller = createCaller(await createTestTRPCContext(user));
      const result = await caller.mailbox.conversations.list({
        ...defaultParams,
        status: ["open"],
        sort: "highest_value",
      });
