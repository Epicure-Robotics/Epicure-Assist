...
test.describe("Customer Settings", () => {
  ...
  test("VIP customers UI states and all functionality", async ({ page }) => {
    ...
    await expect(page.getByText("VIP Priority Threshold", { exact: true })).not.toBeVisible();

    // Test all UI elements + functionality together
    const thresholdInput = page.getByRole("spinbutton", { name: "VIP Priority Threshold", exact: true });
    ...
    await expect(page.getByText("VIP Priority Threshold", { exact: true })).toBeVisible();
