...
describe("createReply", () => {
  it("creates a reply and closes the conversation", async () => {
    ...
    expect(mockTriggerEvent).toHaveBeenCalledWith("gmail/archive-thread", {
      conversationId: conversation.id,
    });
    const gmailArchiveCalls = mockTriggerEvent.mock.calls.filter(([event]) => event === "gmail/archive-thread");
    expect(gmailArchiveCalls).toHaveLength(1);
  });
