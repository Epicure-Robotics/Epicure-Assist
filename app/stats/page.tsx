...
export default function StatsPage() {
  ...
  const [selectedIssueGroupId, setSelectedIssueGroupId] = useState<number | null>(null);
  ...
  const { data: commonIssuesData } = api.mailbox.commonIssuesStats.useQuery({ days: selectedDays });
  const { data: subcategoryData } = api.mailbox.commonIssueSubcategoryStats.useQuery({
    days: selectedDays,
    issueGroupId: selectedIssueGroupId ?? undefined,
  });
  const { data: notRepliedData } = api.mailbox.notRepliedStats.useQuery();
  ...
  return (
    <div className="min-h-screen bg-[hsl(0_58%_10%)] text-white -m-8 p-8 relative">
      ...
      <main className="space-y-8 max-w-screen-lg mx-auto" data-testid="stats-main-content">
        ...
        {/* Not Replied Cases - 24+ Hours */}
        {notRepliedData && (
          <section className="mt-8" data-testid="not-replied-section">
            <h2 className="text-3xl font-bold text-amber-100 mb-4 text-center">Not Replied in 24+ Hours</h2>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
              <Card className="text-center bg-[#331111] border-none text-white">
                <CardHeader>
                  <CardTitle className="text-xl text-amber-100">All</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-6xl font-bold text-bright">{notRepliedData.all}</div>
                </CardContent>
              </Card>

              <Card className="text-center bg-[#331111] border-none text-white">
                <CardHeader>
                  <CardTitle className="text-xl text-amber-100">Mine</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-6xl font-bold text-[#FF90E8]">{notRepliedData.mine}</div>
                </CardContent>
              </Card>

              <Card className="text-center bg-[#331111] border-none text-white">
                <CardHeader>
                  <CardTitle className="text-xl text-amber-100">Assigned</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-6xl font-bold text-[#459EFD]">{notRepliedData.assigned}</div>
                </CardContent>
              </Card>

              <Card className="text-center bg-[#331111] border-none text-white">
                <CardHeader>
                  <CardTitle className="text-xl text-amber-100">Unassigned</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-6xl font-bold text-[#FF4343]">{notRepliedData.unassigned}</div>
                </CardContent>
              </Card>
            </div>
          </section>
        ...
        {/* Common Issues Stats */}
        {commonIssuesData && commonIssuesData.issueGroups.length > 0 && (
          <section className="mt-8" data-testid="common-issues-section">
            <h2 className="text-3xl font-bold text-amber-100 mb-4 text-center">Solved Cases by Common Issue</h2>
            <Card className="bg-[#331111] border-none text-white">
              <CardContent className="pt-6">
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={commonIssuesData.issueGroups}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value: number, name: string) => {
                        if (name === "closedCount") return [value, "Closed"];
                        if (name === "totalCount") return [value, "Total"];
                        return [value, name];
                      }}
                    />
                    <Bar dataKey="closedCount" radius={[8, 8, 0, 0]}>
                      {commonIssuesData.issueGroups.map((entry) => (
                        <Cell
                          key={`cell-${entry.id}`}
                          fill={entry.color || "#FDE68A"}
                          onClick={() =>
                            setSelectedIssueGroupId((prev) => (prev === entry.id ? null : entry.id))
                          }
                          cursor="pointer"
                        />
                      ))}
        ...
        {selectedIssueGroupId && (subcategoryData?.subcategories.length ?? 0) > 0 && (
          <section className="mt-8" data-testid="common-issue-subcategories-section">
            <h2 className="text-2xl font-bold text-amber-100 mb-4 text-center">Subcategories (Drill-down)</h2>
            <Card className="bg-[#331111] border-none text-white">
              <CardContent className="pt-6">
                <div className="flex justify-end mb-2">
                  <button
                    className="text-xs text-amber-100 hover:underline"
                    onClick={() => setSelectedIssueGroupId(null)}
                  >
                    Clear selection
                  </button>
                </div>
                <ResponsiveContainer width="100%" height={320}>
                  <BarChart data={subcategoryData?.subcategories ?? []}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255, 255, 255, 0.1)" />
                    <XAxis
                      dataKey="title"
                      tick={{ fill: "#FEF3C7" }}
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: "#FEF3C7" }}
                      formatter={(value: number, name: string) => [value, name === "closedCount" ? "Closed" : "Total"]}
                    />
                    <Bar dataKey="closedCount" radius={[8, 8, 0, 0]}>
                      {(subcategoryData?.subcategories ?? []).map((entry) => (
                        <Cell key={`subcategory-${entry.id}`} fill={entry.issueGroupColor || "#FDE68A"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
