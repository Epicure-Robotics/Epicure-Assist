...
const KnowledgeBankItem = ({ faq, suggestedReplacement, onDelete }: KnowledgeBankItemProps) => {
  ...
  const promoteMutation = api.mailbox.faqs.promoteToSavedReply.useMutation({
    onSuccess: () => {
      toast.success("Added to saved replies");
      setPromoteName(null);
    },
    onError: () => {
      toast.error("Error promoting to saved reply");
    },
  });
      ...
      {editingContent ? (
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            if (promoteName.trim()) {
              promoteMutation.mutate({ id: faq.id, name: promoteName.trim() });
            }
          }}
        >
          <Input
            autoFocus
            placeholder="Name for saved reply..."
            value={promoteName}
            onChange={(e) => setPromoteName(e.target.value)}
            className="flex-1"
          />
          <Button type="submit" size="sm" disabled={!promoteName.trim() || promoteMutation.isPending}>
            {promoteMutation.isPending ? "Saving..." : "Save"}
          </Button>
          <Button type="button" variant="ghost" size="sm" onClick={() => setPromoteName(null)}>
            Cancel
          </Button>
        </form>
      ) : (
        <div className="flex gap-4">
          ...
          >
            ...
            <div className="flex gap-1 mt-1">
              {suggestedReplacement && (
                <Badge variant="bright" className="shrink-0">
                  Suggested Edit
                </Badge>
              )}
              {(faq.usageCount ?? 0) >= PROMOTE_USAGE_THRESHOLD && (
          ...
          {(faq.usageCount ?? 0) >= PROMOTE_USAGE_THRESHOLD && !suggestedReplacement && (
            <Button
              variant="ghost"
              size="sm"
              iconOnly
              onClick={() => setPromoteName(truncate(faq.content, { length: 50 }))}
              title="Promote to saved reply"
            >
              <ArrowUpFromLine className="h-4 w-4" />
              <span className="sr-only">Promote to saved reply</span>
            </Button>
          )}
          <ConfirmationDialog
