...
export function TicketCommandBar({ open, onOpenChange, onInsertReply, onToggleCc, inputRef }: TicketCommandBarProps) {
  ...
  const handleShowVariableDialog = useCallback((savedReply: SavedReply) => {
    setSelectedSavedReply(savedReply);
  ...
  const handleVariableSubmit = useCallback(
    ...
    [selectedSavedReply, onInsertReply, incrementSavedReplyUsage],
  );
  ...
  const { commandGroups: mainPageGroups } = useMainPage({
    onOpenChange,
    setPage,
    setSelectedItemId,
    onToggleCc,
    setSelectedTool,
    onInsertReply,
    onOpenGenerateDraftDialog: () => setGenerateDraftDialogOpen(true),
  ...
  return open && page === "notes" ? (
    ...
    <>
      ...
      <GenerateDraftDialog
        open={generateDraftDialogOpen}
        onOpenChange={setGenerateDraftDialogOpen}
        onGenerate={(customPrompt) => {
          generateDraft({ conversationSlug, customPrompt });
        }}
      />
      {selectedSavedReply && (
        <TemplateVariableDialog
          ...
          variables={selectedSavedReply.variables || []}
          onSubmit={handleVariableSubmit}
          templateName={selectedSavedReply.name}
        />
      )}
    </>
  );
