...
export const MessageThread = ({
  ...
  // Find indices of first and last regular messages (not events or guide sessions)
  const regularMessageIndices = conversation.messages
    .map((msg, idx) => ({ msg, idx }))
    .filter(({ msg }) => msg.type !== "event" && msg.type !== "guide_session")
    .map(({ idx }) => idx);

  const firstMessageIndex = regularMessageIndices[0] ?? -1;
  const lastMessageIndex = regularMessageIndices[regularMessageIndices.length - 1] ?? -1;
    ...
    <div className="flex h-full flex-col" data-testid="message-thread">
      <div className="flex flex-1 flex-col gap-8">
        ...
        {conversation.messages.map((message, index) =>
          message.type === "event" ? (
            <EventItem key={message.id} event={message} initialExpanded={true} />
            ...
            <ToolItem
              ...
              initialExpanded={index === lastMessageIndex}
            />
          ) : (
            <MessageItem
              ...
              initialExpanded={index === firstMessageIndex || index === lastMessageIndex}
              onPreviewAttachment={
