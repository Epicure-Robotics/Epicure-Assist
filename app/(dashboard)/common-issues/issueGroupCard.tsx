import { Bookmark, BookmarkCheck, Bot } from "lucide-react";
...
interface IssueGroupCardProps {
  group: {
    ...
    vipCount?: number;
    color?: string | null;
    autoResponseEnabled?: number | null;
    topSubcategories?: {
      id: number;
      title: string;
      periodCount: number;
  ...
  return (
    <Link href={`/all?issueGroupId=${group.id}&status=open`} className="block h-full">
      <div className="group relative cursor-pointer h-full" style={{ perspective: "1000px" }}>
        ...
        <Card
          className="relative z-10 transition-all transform origin-top-right duration-300 ease-out flex flex-col hover:shadow-xl cursor-pointer h-full bg-background border"
          style={group.color ? { borderTopColor: group.color, borderTopWidth: "3px" } : undefined}
        >
          <CardHeader className="pb-3 flex-1">
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  <CardTitle className="text-lg font-semibold line-clamp-2 flex-1 flex items-center gap-2">
                    {group.autoResponseEnabled === 1 && (
                      <Bot className="h-4 w-4 text-primary shrink-0" aria-label="Auto-response enabled" />
                    )}
                    <span>
          ...
          <CardContent className="pt-0">
            ...
            {(group.topSubcategories?.length ?? 0) > 0 && (
              <div className="space-y-1.5">
                <div className="text-xs text-muted-foreground">Top subcategories (30d)</div>
                {group.topSubcategories?.map((subcategory) => (
                  <div key={subcategory.id} className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-muted-foreground shrink-0">
                      {subcategory.periodCount} ({Math.round(subcategory.sharePercent)}%)
                    </span>
                  </div>
                ))}
              </div>
